...
const Login = () => {
  ...
  const adminlog = async (e) => {
    e.preventDefault();
    try {
      const { data: admin } = await axios.get(`http://localhost:8090/admin?email=${adminemail}`);
      const user = admin[0];
      if (user) {
        if (user.password === adminpass) {
          toast.success("Login Successful",{duration:200});
          setTimeout(() => {
            console.log("delay")
          }, 3000);
  ...
  return (
    <div className="min-h-screen min-w-full bg-white flex justify-center items-center absolute top-0">
      ...
      <div className="bg-white p-8 rounded-lg shadow-lg relative">
        <div className="flex justify-center font-rajdhani text-2xl">
